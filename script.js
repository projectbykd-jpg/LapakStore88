...
function loadData() {
  fetch(SPREADSHEET_URL)
    ...
    .catch(err => {
      ...
      // Show friendly message if productGrid exists
...
window.filterProductsByName = function(){
  ...
  const grid = document.getElementById('productGrid');
  if (!grid) return;
  const cards = Array.from(grid.querySelectorAll('.ui-card'));
  ...
  document.getElementById('searchNotFound').style.display = anyVisible ? 'none' : 'flex';
}
...
window.openProductModal = function(id){
  ...
  nameEl.innerText = p.name || 'Produk';
  container.innerHTML = '';
  ...
  (p.packets || []).forEach((pkt, i) => {
    ...
    `;
    row.querySelector('input')?.addEventListener('change', () => updateTotalPrice(pkt.price));
    container.appendChild(row);
  });

  totalEl.innerText = (p.packets && p.packets[0]) ? formatPrice(p.packets[0].price) : (p.price ? formatPrice(p.price) : 'Rp 0');

  modal.classList.add('active');
  modal.setAttribute('aria-hidden','false');

  // focus first radio if possible for keyboard users
...
// Novel rendering (kepler from previous implementation, kept safe)
...
window.openNovelModal = function(id){
  // existing logic kept minimal — ensure elements exist
  const active = novelsData.find(x => String(x.id) === String(id));
  if (!active) return;
  // inject modal content if modal present
  const modal = document.getElementById('novelModal');
  if (!modal) return;
  document.getElementById('modalNovelTitle').innerText = active.judul || 'Novel';
...
window.readChapter = function(i){
  const active = novelsData.find(n => n.chapters && n.chapters.find((_,idx)=>idx===i));
  if (!active) return;
